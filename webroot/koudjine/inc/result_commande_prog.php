...
if (isset($_GET["motclef1"])) {
    ...
    $sth = $pdo->prepare("
              ...
              WHERE p.nom like :motclef AND p.grossiste_id is null AND p.id = r.produit_id AND p.supprimer = 0 AND r.dateLivraison IN (select max(dateLivraison) from en_rayon e where r.produit_id = e.produit_id )

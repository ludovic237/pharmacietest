...
if (isset($_POST["motclef"])) {
    ...
    $sth = $pdo->prepare("
              SELECT p.nom, c.quantite, c.prixUnit, v.id as idv, r.id as idr
    ...
    if ($count) {
        while ($result = $sth->fetch(PDO::FETCH_OBJ)) {
            echo "<tr data='".$result->idv."' id=\"R".$result->idr."\">
                                            ...
                                            <td>
                                                <button class=\"btn btn-primary \" data-toggle=\"tooltip\" data-placement=\"top\" onclick=\"load_produit_retour('".$result->idr."','".$result->idv."')\"><span class=\"\">Charger</span></button>
                                            </td>
                                        </tr>";

...
if (isset($_GET["motclef1"])) {
    ...
    $sth = $pdo->prepare("
              SELECT p.nom, p.stock, r.quantite, r.reduction, p.reductionMax, r.prixAchat, r.id as id, r.dateLivraison, p.id as idp 
        ...
        if ($count) {
            while ($result = $sth->fetch(PDO::FETCH_OBJ)) {
                ...
                echo "<tr id=\"R" . $result->idp . "\">
                                            <td class='nom'><strong>" . $result->nom . " ( " . $result->stock . " )</strong></td>
                                            <td class='type'>En rayon</td>

                                            <td>
                                                <button class=\"btn btn-primary \" data-toggle=\"tooltip\" data-placement=\"top\" onclick=\"load_produit_inventaire('" . $result->idp . "','en rayon')\"><span class=\"\">Charger</span></button>
                                            </td>
                                        </tr>";
        ...
        if ($count1) {
            while ($result1 = $sth1->fetch(PDO::FETCH_OBJ)) {
                ...
                echo "<tr id=\"R" . $result1->id . "\">
                                            <td class='nom'><strong>" . $result1->nom . " ( " . $result1->stock . " )</strong></td>
                                            <td class='type'>Detail</td>
                                            <td>
                                                <button class=\"btn btn-primary \" data-toggle=\"tooltip\" data='detail' data-placement=\"top\" onclick=\"load_produit_inventaire('" . $result1->id . "','detail')\"><span class=\"\">Charger</span></button>
                                            </td>
                                        </tr>";
                //echo "<li  style=\"background-color: #fff; list-style-type: none; margin: 0; padding: 0;\"><tr><a href=\"update/".$result->id."\" style=\"display:block; height: 25px; color: #000; text-decoration: none;\"><td>$result->nom</td></a><td>$result->stock</td><tr>$result->reductionMax</tr></li>";
            }
